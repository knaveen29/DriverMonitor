...
{
    public class SingleDriveViolation : IViolationFactory
    {
        private readonly DriverActivityViolation violation = new DriverActivityViolation();

        private readonly ViolationType violationType = ViolationType.SingleDrive;

        public SingleDriveViolation()
        {
        ...
        public DriverActivityViolation GetViolationData(List<DriverActivityFile> activities)
        {
            foreach (var activity in activities.Where(_ => _.Activity == "Drive").ToList())
            {
                ...
                if ((activity.EndTime - activity.StartTime).TotalHours > 4 && (activity.EndTime - activity.StartTime).TotalHours < 12)
                {
